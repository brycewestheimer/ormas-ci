...
def test_unrestricted_matches_casci(h2_rhf, h2_ormas_config):
    ...
    """
    # PySCF reference
    mc_ref = mcscf.CASCI(h2_rhf, 2, 2)
    mc_ref.verbose = 0
    e_ref = mc_ref.kernel()[0]

    # Our solver, no restrictions
    mc_test = mcscf.CASCI(h2_rhf, 2, 2)
    mc_test.verbose = 0
    mc_test.fcisolver = ORMASFCISolver(h2_ormas_config)
    e_test = mc_test.kernel()[0]

    assert abs(e_ref - e_test) < 1e-10, (
        f"Energy mismatch: PySCF={e_ref}, ORMAS={e_test}, diff={abs(e_ref - e_test)}"
    )
...
def test_unrestricted_matches_casci_h2o():
    ...
    assert abs(e_ref - e_test) < 1e-10, (
        f"H2O energy mismatch: PySCF={e_ref}, ORMAS={e_test}, diff={abs(e_ref - e_test)}"
    )
...
def test_restricted_energy_above_casci():
    ...
    assert e_test >= e_ref - 1e-10, (
        f"Variational violation: restricted={e_test} < CASCI={e_ref}, diff={e_test - e_ref}"
    )


def test_rdm1_from_fcisolver(h2_rhf, h2_ormas_config):
    ...
    assert abs(np.trace(rdm1) - 2.0) < 1e-10, f"RDM1 trace = {np.trace(rdm1)}, expected 2.0"


def test_determinant_reduction_logged(caplog, h2_rhf, h2_ormas_config):
