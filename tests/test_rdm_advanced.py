...
def test_rdm2_symmetry_aa():
    ...
    (_, _), (rdm2aa, _, _) = mc.fcisolver.make_rdm12s(mc.ci, ncas, nelecas)
    diff = rdm2aa + rdm2aa.transpose(2, 1, 0, 3)
    assert np.allclose(diff, 0, atol=1e-12), (
        f"rdm2aa antisymmetry violated, max |err| = {np.max(np.abs(diff))}"
    )
...
def test_rdm2_symmetry_bb():
    ...
    mf, ncas, nelecas, config = _h2o_setup()
    mc = _run_casci(mf, ncas, nelecas, config)
    (_, _), (_, _, rdm2bb) = mc.fcisolver.make_rdm12s(mc.ci, ncas, nelecas)
    diff = rdm2bb + rdm2bb.transpose(2, 1, 0, 3)
    assert np.allclose(diff, 0, atol=1e-12), (
        f"rdm2bb antisymmetry violated, max |err| = {np.max(np.abs(diff))}"
    )
...
def test_rdm2_trace():
    ...
    assert abs(trace - expected) < 1e-10, f"Tr(rdm2) = {trace}, expected {expected}"
...
def test_rdm2_energy_h2():
    ...
    e_rdm = (
        np.einsum("pq,qp", mc._h1e, rdm1) + 0.5 * np.einsum("pqrs,pqrs", mc._h2e, rdm2) + mc._ecore
    )
    assert abs(e_kernel - e_rdm) < 1e-10, f"H2 energy mismatch: kernel={e_kernel}, rdm={e_rdm}"
...
def test_rdm2_energy_h2o():
    ...
    assert abs(e_kernel - e_rdm) < 1e-10, f"H2O energy mismatch: kernel={e_kernel}, rdm={e_rdm}"
...
def test_rdm2_matches_pyscf_h2():
    ...
    # PySCF FCI reference
    e_fci, ci_fci = fci.direct_spin1.kernel(mc._h1e, mc._h2e, ncas, nelecas)
...
def test_rdm12s_consistency():
    ...
    mc = _run_casci(mf, ncas, nelecas, config)
    _, rdm2 = mc.fcisolver.make_rdm12(mc.ci, ncas, nelecas)
    (_, _), (rdm2aa, rdm2ab, rdm2bb) = mc.fcisolver.make_rdm12s(mc.ci, ncas, nelecas)
    rdm2_sum = rdm2aa + rdm2ab + rdm2ab.transpose(2, 3, 0, 1) + rdm2bb
    assert np.allclose(rdm2, rdm2_sum, atol=1e-12), (
        f"rdm2 != rdm2aa + rdm2ab + rdm2ba + rdm2bb, max |err| = {np.max(np.abs(rdm2 - rdm2_sum))}"
    )
...
def test_rdm1s_from_rdm12s():
    ...
    mf, ncas, nelecas, config = _h2o_setup()
    mc = _run_casci(mf, ncas, nelecas, config)
    rdm1a_direct, rdm1b_direct = mc.fcisolver.make_rdm1s(mc.ci, ncas, nelecas)
    (rdm1a_12, rdm1b_12), _ = mc.fcisolver.make_rdm12s(mc.ci, ncas, nelecas)
...
def test_rdm1s_sum_equals_rdm1():
    ...
    assert np.allclose(rdm1, rdm1a + rdm1b, atol=1e-12), "rdm1 != rdm1a + rdm1b"
...
def test_rdm1s_trace():
    ...
    rdm1a, rdm1b = mc.fcisolver.make_rdm1s(mc.ci, ncas, nelecas)
    na, nb = nelecas
    assert abs(np.trace(rdm1a) - na) < 1e-10, f"Tr(rdm1a) = {np.trace(rdm1a)}, expected {na}"
    assert abs(np.trace(rdm1b) - nb) < 1e-10, f"Tr(rdm1b) = {np.trace(rdm1b)}, expected {nb}"
...
def test_rdm2_restricted_ormas():
    ...
    assert abs(e_kernel - e_rdm) < 1e-10, (
        f"Restricted ORMAS energy mismatch: kernel={e_kernel}, rdm={e_rdm}"
    )
...
def test_transform_identity():
    ...
    mc = _run_casci(mf, ncas, nelecas, config)
    u = np.eye(ncas)
    ci_new = mc.fcisolver.transform_ci_for_orbital_rotation(mc.ci, ncas, nelecas, u)
    assert np.allclose(ci_new, mc.ci, atol=1e-12), (
        f"Identity transform changed CI, max |diff| = {np.max(np.abs(ci_new - mc.ci))}"
    )
...
def test_transform_roundtrip():
    ...
    ci_back = mc.fcisolver.transform_ci_for_orbital_rotation(ci_rot, ncas, nelecas, u.T)
    assert np.allclose(ci_back, mc.ci, atol=1e-10), (
        f"Round-trip failed, max |diff| = {np.max(np.abs(ci_back - mc.ci))}"
    )
...
def test_transform_open_shell():
    ...
    ci_back = mc.fcisolver.transform_ci_for_orbital_rotation(ci_rot, ncas, nelecas, u.T)
    assert np.allclose(ci_back, mc.ci, atol=1e-10), (
        f"CH2 round-trip failed, max |diff| = {np.max(np.abs(ci_back - mc.ci))}"
    )
...
def test_contract_matches_hamiltonian():
    ...
    h_ci = build_ci_hamiltonian(alpha_strings, beta_strings, mc._h1e, mc._h2e)
    ...
    h2e_absorbed = solver.absorb_h1e(mc._h1e, mc._h2e, ncas, nelecas, fac=1)
    sigma = solver.contract_2e(h2e_absorbed, mc.ci, ncas, nelecas)

    assert np.allclose(sigma, sigma_ref, atol=1e-10), (
        f"contract_2e mismatch, max |err| = {np.max(np.abs(sigma - sigma_ref))}"
    )
...
def test_absorb_h1e_matches_pyscf():
    ...
    h2e_ours = solver.absorb_h1e(mc._h1e, mc._h2e, ncas, nelecas, fac=0.5)

    h2e_ref = fci.direct_spin1.absorb_h1e(mc._h1e, mc._h2e, ncas, nelecas, fac=0.5)
    ...
    assert np.allclose(h2e_ours_full, h2e_ref_full, atol=1e-12), (
        f"absorb_h1e mismatch, max |err| = {np.max(np.abs(h2e_ours_full - h2e_ref_full))}"
    )
...
def test_large_ci_returns_dominant():
    ...
    for coeff, a_str, b_str in result:
        assert abs(coeff) > 0.1, f"Coefficient {coeff} does not exceed tol=0.1"
...
def test_large_ci_threshold():
    ...
    n_det = len(mc.ci)
    assert len(result) == n_det, f"tol=0 returned {len(result)} configs, expected {n_det}"
...
def test_casscf_h2_runs():
    ...
    mc.fcisolver = ORMASFCISolver(config)
    mc.kernel()
    assert mc.converged or mc.e_tot < 0, "CASSCF on H2 did not converge and energy is non-negative"
...
def test_casscf_h2o_runs():
    ...
    mc.fcisolver = ORMASFCISolver(config)
    mc.kernel()
    assert mc.converged or mc.e_tot < 0, "CASSCF on H2O did not converge and energy is non-negative"
...
def test_casscf_energy_leq_casci():
    ...
    assert e_casscf <= e_casci + 1e-8, f"CASSCF energy ({e_casscf}) > CASCI energy ({e_casci})"
