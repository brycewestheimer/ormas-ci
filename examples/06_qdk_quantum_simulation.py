...
as_sel = create(
    ...
    num_active_electrons=sum(nelecas), num_active_orbitals=ncas,
)
...
print(f"Formaldehyde CAS(4,3)/cc-pVDZ: {qubit_ham.num_qubits} qubits, "
...
cas_prep = build_state_prep(
    np.array(ci_flat), np.array(a_flat, dtype=np.int64),
    np.array(b_flat, dtype=np.int64), ncas, qdk_orbs,
)

# ORMAS: use the restricted CI vector directly
ormas_prep = build_state_prep(
    mc_ormas.fcisolver.ci, mc_ormas.fcisolver._alpha_strings,
    mc_ormas.fcisolver._beta_strings, ncas, qdk_orbs,
)
...
for label, prep in [("CASCI", cas_prep), ("ORMAS", ormas_prep)]:
    pe = create(
        ...
        num_bits=6, evolution_time=evolution_time, shots_per_bit=100,
    )
    result = pe.run(
        prep, qubit_ham, evolution_builder=trotter,
        circuit_mapper=mapper, circuit_executor=executor,
    )
...
print(f"{'ORMAS':<8} {n_det_ormas:>6} {e_ormas:>16.8f} "
      f"{(e_ormas - e_casci) * 1000:>+10.3f}")
print()
